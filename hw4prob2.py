...
psc_service = psc_services['IRSA TAP'] # IRSA Table Access Protocol (TAP) Service
...
target_list = ['M2',
               ...
               'NGC 1068',
               'AU Mic',
               'TRAPPIST-1',  # this one has J = 11.4 mag
               ]
for target in target_list:
    ...
    rad = 6. / 60. # [convert from arcmin to deg]  cone search radius
    jlim = 12. # [mag]  J-band magnitude limit
    ...
    # read this: https://irsa.ipac.caltech.edu/docs/program_interface/TAP.html
...
AND j_m <= {}
'''.format(coords.ra.deg, coords.dec.deg, rad, jlim)  # the .format suffix autofills "{}" in order
    ...
    ax.scatter(x, y,
               marker='o',
               edgecolor='red',  # edgecolor = 'k' = 'black'. Change it so it's easier to see
               facecolor='none',
               )
